...
const strOfThe_Strings_ToBe_Ignored = ".git,.vscode,.js,.ts,.cs,.html,.css,.java,.kt,./the_sorted_database/${customer_job}.json";
...
async function check_The_Existence_OFThe_Paths_That_Are_Inside_The_Files_Of_This_Directory() {
  ...
  if (Object.keys(obj_OfThe_Paths_That_DoesNot_Exist_And_Their_MetaData).length == 0) {
    ...
    const mistakes_Count = Object.keys(obj_OfThe_Paths_That_DoesNot_Exist_And_Their_MetaData).length;
    console.log(
      "\x1b[31m%s\x1b[0m",
...
async function runTheCheck_On_The_Targeted_Directory_And_Its_SubDirectories(
  ...
  const arrOfDirents_OfAll_The_Files_And_The_Directories_Inside_The_Targeted_Directory = await fsPromises.readdir(
    the_Path_OfThe_Targeted_Directory_That_Need_ToBe_Checked,
    {
      withFileTypes: true,
    }
  ...
  for (const dirent of arrOfDirents_OfAll_The_Files_And_The_Directories_Inside_The_Targeted_Directory) {
    if (dirent.isDirectory()) {
      ...
      await runTheCheck_On_Only_Sub_Directories(dirent.name, the_Path_OfThe_Targeted_Directory_That_Need_ToBe_Checked);
      //#endregion
    } else {
      ...
      await run_The_Actual_File_Checking(dirent.name, the_Path_OfThe_Targeted_Directory_That_Need_ToBe_Checked);
      //#endregion
    }
  }
  //#endregion
...
async function runTheCheck_On_Only_Sub_Directories(
  ...
  await runTheCheck_On_The_Targeted_Directory_And_Its_SubDirectories(path_OfThe_SubDir_That_Is_Under_Check);
  //#endregion
}
//------------------------------------------------------------------------------------------------------------
/**
 ...
 */
async function run_The_Actual_File_Checking(the_File_Name, the_Path_OfThe_Directory_That_Is_Under_Check) {
  //#region some needed variables
  const the_Path_OfThe_File_ThatIs_Under_Checking = path.join(the_Path_OfThe_Directory_That_Is_Under_Check, the_File_Name);
  //#endregion
  try {
    //#region get all the Acceptable PathLike_Strings from the current file that is under checking
    const the_Content_OfThe_File_Under_Checking = await fsPromises.readFile(the_Path_OfThe_File_ThatIs_Under_Checking, "utf-8");
    const arr_OfAll_The_Accepted_PathLike_Strings = giveMe_The_Arr_Of_The_Acceptable_PathLike_Strings(
      the_Content_OfThe_File_Under_Checking
    );
    //#endregion

    //#region get the PathLike_Strings that you want to ignore
    let arr_OfThe_Strings_ToBe_Ignored = strOfThe_Strings_ToBe_Ignored.split(/[,\n\r\t]/gim).filter((item) => {
    ...
    arr_OfAll_The_Accepted_PathLike_Strings.forEach((pathLike_Str) => {
      ...
      if (/\W\D$/gim.test(pathLike_Str) && /^\.+\/|^.+:\/\/|^.+:\\\\/gim.test(pathLike_Str) == false) return;
...
function giveMe_The_Arr_Of_The_Acceptable_PathLike_Strings(the_Content_OfThe_File_Under_Checking) {
  ...
  const arr_OfThe_Indexes_OfThe_Double_Quotes_Inside_The_Current_File_Under_Checking =
    giveMe_The_IndexesArr_OfAllThe_Occurrences_Of_A_QuotationMark_Inside_A_String(`"`, the_Content_OfThe_File_Under_Checking);
 ...
 * @param {string} the_Content_OfThe_File_Under_Checking
 */
function giveMe_The_Arr_Of_All_The_Possible_Strings(arr_ofThe_ArraysOfIndexes, the_Content_OfThe_File_Under_Checking) {
  ...
  arr_ofThe_ArraysOfIndexes.forEach((arr_Of_Indexes_OfThe_Quotation_Marks) => {
    ...
    arr_Of_Indexes_OfThe_Quotation_Marks.forEach((indexOfThe_Quotation_Mark, index_Of_This_IndexOfTheMatch) => {
      ...
      const the_Next_IndexOfTheMatch = arr_Of_Indexes_OfThe_Quotation_Marks.at(index_Of_This_IndexOfTheMatch + 1);
      ...
      arr_OfTheStrings_From_Inside_The_Content_OfThe_Current_File_Under_Checking.push(the_Current_SubStr);
      //#endregion
    });
    //#endregion
  });
...
function giveMe_The_IndexesArr_OfAllThe_Occurrences_Of_A_QuotationMark_Inside_A_String(
  ...
  for (;;) {
    ...
    const the_Index_OfThe_Occurrence = the_Content_OfThe_File_Under_Checking.indexOf(the_Quotation_Mark, the_Start);
  ...
  if (arr_OfThe_Indexes_OfThe_Occurrences.length % 2 != 0) {
    arr_OfThe_Indexes_OfThe_Occurrences.push(the_Content_OfThe_File_Under_Checking.length - 1);
  }
  //#endregion
  //#endregion
...
async function handle_Only_A_Single_Array(
  ...
  const arr_OfThe_Lines_OfThe_Content_OfThe_File_That_Under_Checking = the_Content_OfThe_File_Under_Checking.split("\n");
  //#endregion
  for (let pathLike_String of the_Array) {
    ...
    try {
      ...
      obj_1["the_Path_OfThe_File_That_Has_The_Problem"] = the_Path_OfThe_File_Under_Check;
      //#endregion
      ...
      arr_OfThe_Lines_OfThe_Content_OfThe_File_That_Under_Checking.forEach((line, index_OfThe_Line) => {
        if (line.includes(pathLike_String) == false) return;
        if (new RegExp(`(?<![\"\'\`].*)//.*[\"\'\`]${pathLike_String}`, "gmi").test(line)) {
          delete obj_OfThe_Paths_That_DoesNot_Exist_And_Their_MetaData[counter_1];
          counter_1--;
          return;
        }

        str_The_Numbers_OfThe_Lines_OfThe_Non_Existed_Paths += ` ${index_OfThe_Line + 1}`;
      });

      obj_1["the_Numbers_OfThe_Lines"] = str_The_Numbers_OfThe_Lines_OfThe_Non_Existed_Paths;
