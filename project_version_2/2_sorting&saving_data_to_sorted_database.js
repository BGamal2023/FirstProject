...
//#endregion
//#region functions
export function m1_tell_me_how_many_customers_in_every_job() {
  //#region get array of all customers jobs

  for (let i = 0; i < arr_of_JAVAscript_objects_from_db.length; i++) {
    let customers_job = arr_of_JAVAscript_objects_from_db[i].job;
    if (!arr_of_customers_jobs.includes(customers_job)) {
      arr_of_customers_jobs.push(customers_job);
    }
  }
  ...
  for (let i = 0; i < arr_of_JAVAscript_objects_from_db.length; i++) {
    for (let j = 0; j < arr_of_JAVAscript_objects_from_db.length; j++) {
      if (arr_of_JAVAscript_objects_from_db[j].job == arr_of_customers_jobs[i]) {
        s_customer_obj = {
